...
pub(crate) fn extract_files_from_patch(patch_text: &str) -> Vec<String> {
    match codex_apply_patch::parser::parse_patch(patch_text) {
        Ok(parsed) => parsed
            ...
            .iter()
            .map(|hunk| match hunk {
                codex_apply_patch::parser::Hunk::AddFile { path, .. }
                | codex_apply_patch::parser::Hunk::DeleteFile { path }
                | codex_apply_patch::parser::Hunk::UpdateFile { path, .. } => {
...
pub(crate) fn is_apply_patch_command(command: &[String], cwd: &Path) -> bool {
    matches!(
